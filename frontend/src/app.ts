...
function initTextSelectionToolbar(): void {
  document.addEventListener("mouseup", () => {
    setTimeout(() => {
      ...
      const d0 = document.createElement("span"); d0.className = "tst-divider"; toolbar.appendChild(d0);
...
/**
 * rag_call_rounds diagnostics (Retriever Task #80 / Chat #37): a multi-CALL RAG sequence — one entry
 * per rewritten-query RAG call — shown as collapsible "Call N" rows, each carrying that call's query,
 * terminal_action outcome, latency, and its own per-stage module_trace. Preferred over the single
 * renderModuleTrace when the backend emits rag_call_rounds (it did — ReAct #80 shipped). Single-call
 * turns render one row. Returns null when the field is absent (→ caller falls back to renderModuleTrace).
 */
function renderRagCallRounds(thinkingLog: ReadonlyArray<unknown> | null | undefined): HTMLElement | null {
  if (!Array.isArray(thinkingLog)) return null;
  let rounds: Array<Record<string, unknown>> | null = null;
  for (const entry of thinkingLog) {
    const dd = entry && typeof entry === "object" ? (entry as { data?: unknown }).data : null;
    const rr = dd && typeof dd === "object" ? (dd as { rag_call_rounds?: unknown }).rag_call_rounds : null;
    if (Array.isArray(rr) && rr.length) rounds = rr as Array<Record<string, unknown>>;   // last wins (final trace)
  }
  ...
  rounds.forEach((r, i) => {
    ...
    const lat = (r.latency_ms && typeof r.latency_ms === "object") ? (r.latency_ms as Record<string, unknown>) : {};
    const total = typeof lat.total_ms === "number" ? Math.round(lat.total_ms) : null;
    if (total != null) grandTotal += total;
    const mt = Array.isArray(r.module_trace) ? (r.module_trace as Array<Record<string, unknown>>) : [];

    const row = document.createElement("details");
    row.className = "mt-row";
    const hdr = document.createElement("summary");
    hdr.className = "mt-row-hdr";
    const badge = document.createElement("span"); badge.className = "mt-badge"; badge.textContent = String(i + 1);
    const name = document.createElement("span"); name.className = "mt-name";
    name.textContent = query ? `Call ${i + 1} · “${query}”` : `Round ${n}`;
    const metric = document.createElement("span"); metric.className = "mt-metric";
    const hp: string[] = [];
    if (outcome) hp.push(outcome);
    if (total != null) hp.push(`${total} ms`);
    metric.textContent = hp.join(" · ") || "—";
    ...
    if (query) _dcKV(body, "query", query);
    _dcKV(body, "round", String(n));
    _dcKV(body, "terminal_action", outcome || "—");
    // Per-stage module_trace for THIS call (compact one KV per stage).
    mt.forEach((e) => {
      const stage = typeof e.stage === "string" ? e.stage : "stage";
      const label = e.slot ? `${stage} — slot ${e.slot}` : (e.theme ? `${stage} — ${e.theme}` : stage);
      const bits: string[] = [];
      if (typeof e.ms === "number") bits.push(`${Math.round(e.ms)} ms`);
      if (typeof e.candidates === "number") bits.push(`${e.candidates} cand`);
      if (typeof e.dispatch_path === "string" && e.dispatch_path) bits.push(e.dispatch_path);
      _dcKV(body, String(label), bits.join(" · ") || "—");
    });
    row.appendChild(body);
    acc.appendChild(row);
  });
  ...
  preview.setAttribute("tabindex", "0");
  preview.setAttribute("aria-expanded", "false");
  const titleEl = document.createElement("span"); titleEl.className = "llm-performance-title"; titleEl.textContent = "RAG telemetry";
  const oneline = document.createElement("span"); oneline.className = "llm-performance-oneline";
  oneline.textContent = `${rounds.length} call${rounds.length === 1 ? "" : "s"}${grandTotal > 0 ? " · " + grandTotal + " ms" : ""}`;
...
function run(): void {
  ...
  function _injectDiagnosticsTab(
    ...
    // Prefer the multi-call rag_call_rounds view (Retriever #80 — collapsible Call N rows, each with
    // its own query + module_trace) when the backend emits it; else the single 8-stage module trace.
  ...
  async function _sendMessageAsync(overrideMessage?: string, opts?: SendMessageOpts): Promise<void> {
    ...
    // Collapse all thinking blocks except the most recent one so the last
    // turn's trace stays visible when the next query is submitted.
  ...
  // Bridge for the selection toolbar "Ask about this" action.
  // initTextSelectionToolbar() lives outside this closure, so it
  // dispatches a custom event; we listen here where sendMessage is in scope.
  document.addEventListener("mobius:ask-with-context", (e: Event) => {
