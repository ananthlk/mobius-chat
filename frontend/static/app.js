...
function run() {
  ...
  function _openAtDropdown(anchor, coworkers, atStart, atEnd) {
    _closeAtDropdown();
    if (!coworkers.length)
      return;
    const dd = document.createElement("div");
    ...
    coworkers.forEach((c, i) => {
      ...
      item.innerHTML = `<span class="at-mention-name">${escapeHtml4(c.display_name)}</span>${c.email ? `<span class="at-mention-email">${escapeHtml4(c.email)}</span>` : ""}`;
      item.addEventListener("mousedown", (e) => {
        e.preventDefault();
