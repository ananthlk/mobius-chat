...
## 10. Query-intent reasoning-depth floor (built 2026-08-04)

**Why.** `directive_to_reasoning_depth()` (§8) is purely stage-derived — a round's directive (search/consolidate/extend/finalize) decides its depth, with no awareness of what the user actually asked for. Live testing (Ananth, agentic mode, "produce a detailed summary report... 2 pages with exhibits") surfaced the gap directly: the turn was already in the highest-budget mode, yet every `search` round still got `reasoning_depth="fast"` — identical to what an "is X covered?" one-liner would get in the same mode. Mode selection is the coarse query-intent signal the user already gave us; the per-round depth computation was ignoring it below the synthesize/finalize rounds.

**The combiner.** `app/pipeline/react/governor.py::resolve_reasoning_depth(stage_depth, query_floor)` — floor semantics only. Query intent can RAISE a round's depth above what the directive alone earned; it can never lower it. A `consolidate` round under governor time pressure stays `fast` even on a report query (time pressure is still real); a report query's `search` rounds get pulled up to `thinking` even though `search` alone maps to `fast`. Rank order: `fast(0) < normal(1) < thinking(2)`, picks the max of whichever inputs are non-`None`; `None` when both are `None` (fail-soft, matches every other bandit-criteria function in this module).

**The floor source — `extract_query_intent_floor(message)`.** A keyword heuristic on the raw user message, computed once pre-Round-1 (cheap, no LLM call). Deliberately NOT a Flash/LLM classification step — same reasoning as declining Eval's empirical groundedness pre-read (§9): don't spend a network hop + judge-call latency, on every turn, for a signal a deterministic check gets close enough on. Keyword list is conservative on purpose: `report`, `analysis`, `breakdown`, `detailed`, `comprehensive`, `summary of`, `credentialing report`, `assessment` → `"thinking"`; everything else → `None` (let the stage decide entirely). Chat Architecture's explicit instruction: err toward `None` over a false positive — a missed report query gets normal treatment; a misclassified quick question gets over-allocated `thinking`, which costs latency/money for zero benefit.

**Wiring.** Computed once in `react_loop.py`'s pre-loop setup (`_pp_query_intent_floor`, unconditionally — cheap even when the governor ends up off and nothing reads it), then combined at the exact call site `directive_to_reasoning_depth(directive)` used to feed alone: `resolve_reasoning_depth(directive_to_reasoning_depth(_pp_pre_directive), _pp_query_intent_floor)`.

**Ceiling scaling — `scale_ceiling_for_intent(base_ceiling_s, query_floor)`.** The other half of the same ask ("give report mode really long time limits, not just a better model"). `1.5x` for a `"thinking"` floor, `1.0x` (unchanged) otherwise. Two clamps, not one: the documented `600s` absolute cap, AND the real infra deadline (`_turn_deadline_seconds()`, i.e. `MOBIUS_TURN_DEADLINE_S`) — the governor must never promise more wall-clock than the worker's own background-thread deadline will actually allow. `default_contract_for_mode()` already sets `hard_ceiling_s` to exactly the infra deadline for every mode today (no per-mode headroom below it), so **this scaling has no visible effect until `MOBIUS_TURN_DEADLINE_S` itself is raised above its current dev value (300s)** — an intentional consequence of the safety clamp, not a bug. Wired at contract construction time in `react_loop.py`: `ProductPromiseContract` is frozen, so the scaled contract is built via `dataclasses.replace(_pp_contract, hard_ceiling_s=...)` immediately after `default_contract_for_mode()`, before anything else reads `_pp_contract.hard_ceiling_s`.

**Tests.** `tests/test_governor_bandit_criteria.py::TestResolveReasoningDepth` / `::TestExtractQueryIntentFloor` / `::TestScaleCeilingForIntent` (pure-function coverage, including the critical infra-deadline clamp case) and `::TestQueryIntentFloorEndToEnd` (the exact live scenario through a scripted `run_react()` — a report query's round-1 `search` directive gets pulled up to `thinking`; a quick factual question is unaffected).

**Not done here:** the keyword list is a first pass, not tuned against real query logs. The ceiling scaling has no live effect until `MOBIUS_TURN_DEADLINE_S` is raised — that's an infra/deploy decision, not something this commit changes.
