...
Last updated: 2026-04-18 (after B.1c catalog — uploads now queryable cross-thread; sets foundation for B.1e picker, B.2/B.3 tag review, B.7 batch promotion, and TTL cleanup cron).
...
| B.1c | **Durable instant-RAG upload catalog.** Phase B.1 stored per-thread uploads in a JSONB blob (`chat_state.state_json.active.uploaded_files[]`). That worked for current-thread queries but 2026-04-17 live test exposed the gap: user hard-refreshed browser → new thread → couldn't find the doc uploaded 40 min earlier on a previous thread. New migration 031 + `app/storage/instant_rag_catalog.py` + `app/api/uploads.py` router give cross-thread visibility: `GET /chat/uploads?user_id=X` returns all of that user's uploads regardless of which thread they landed on. `_handle_instant_rag_upload` dual-writes to both JSONB (fast path, unchanged for ReAct) and catalog (durable source of truth). Status field with transitions: active → expired (cleanup cron, future) / discarded (user delete, future) / promoted (Phase B.7). Tag columns (suggested_* / confirmed_*) reserved for Phase B.2/B.3. Default 7-day TTL matches skill's INSTANT_RAG_TTL_DAYS. Backfill script `scripts/backfill_instant_rag_catalog.py` moves existing JSONB history into the catalog; idempotent via `ON CONFLICT DO NOTHING`. 20 new tests: CRUD, status transitions, router contract (thread_id/user_id required, 404 on missing doc), dual-write invariant that locks `append_uploaded_file_record` + `record_upload` both in the upload handler. | `feat(storage): durable instant-rag upload catalog (Phase B.1c)` |
