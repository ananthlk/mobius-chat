...
**Status, 2026-08-08:** ramp completed to 100% and made durable (baked into
`scripts/deploy.sh`'s env vars, commit 3518171) — the staged-ramp discussion
above is closed, this is live at full volume, not a future plan.

## Addendum, 2026-08-08: comprehensive-final re-scope (§§2b, 2)

Sections 1-3 and 2(b) above are now **stale** on two points, per a same-day
follow-up from Ananth (relayed via Chat FE) that reframes what Call A's job is:

**Reframe:** the chat bubble's Summary tab is `react_draft` — ReAct's quick
single-pass. The integrator's Call A output is the FINAL answer, and it must be
**more comprehensive** than the draft, not a same-size reformat of it — every
grounded sub-part covered (e.g. a timely-filing question should surface initial
window + resubmission window + participating-vs-non-participating, not one
line), pulling in `rag_chunks` the draft didn't cite if relevant. Applies to
BOTH modes (FACTUAL and CANONICAL) — no mode gets a shorter bar, and no fixed
section/bullet-count cap on either. Hard guardrail: maximize COVERAGE of real
grounded content, never padding/fabrication — every added sentence must trace
to `reasoning_ledger`/`rag_chunks`/`tool_outputs`; thin grounding still means a
short, honest answer.

This does NOT reopen the "formatter, not synthesizer" contract from §1 above —
Call A still builds ON react's reasoning_ledger/react_draft rather than
re-deriving from raw evidence independently. "Comprehensive" means *expand
against react's own findings* (reflect every round's running_answer, surface
more of the same evidence react already had), not *re-reason from scratch*.
The critic (B) / enrichment (C) scope and prompts are unchanged.

**§2(b) `max_tokens` is now stale:** that section's premise ("Call A's real
output should be substantially smaller" under the formatter framing, reduced
to 2048) is superseded twice over — first by the same-day P0 truncation fix
(widened to 4096, commit ea3bd07, because Gemini 2.5's thinking-token
consumption ate the 2048 budget with no SDK-level control to disable it), then
by this comprehensiveness re-scope (widened again to **8192**, since a
genuinely fuller answer needs real headroom beyond truncation-safety alone).
`latency_budget_ms` for Call A moved 3000 → **4000** to match — still a soft
candidate-eligibility filter, never a request timeout, so this isn't a latency
SLA change, just keeping the router's fast-tier gate roughly proportional to
the larger expected output.
