"""
Minimal HTTP server for Cloud Run: runs the chat worker in a background thread
and serves /health so Cloud Run keeps the instance alive.
Restarts the consumer thread if it exits (e.g. transient Redis/DB failure).
...
app = FastAPI(title="Mobius Chat Worker", version="0.1.0")
...
def start_worker():
    ...
    def _run():
        ...
        while not _stop_restart:
            ...
            t.start()
            logger.info("Chat worker started in background thread")
            t.join()
            if _stop_restart:
                break
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "mobius-chat-worker"}
