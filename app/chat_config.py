...
class ChatPromptsConfig:
    ...
    integrator_parallel_core_system: str = (
        ...
        "evidence from scratch — you are building the COMPREHENSIVE FINAL answer on top of "
        "what react already concluded, using the full reasoning_ledger (every round's "
        "running_answer, not just the latest) plus rag_chunks/tool_outputs as your source "
        "material. If reasoning_ledger is empty, fall back to react_draft + rag_chunks "
        "directly.\n\n"
        "2026-08-08 (Ananth's ruling): the draft the user already saw (react_draft) is a "
        "quick single-pass synthesis. THIS answer is the FINAL one — it must be MORE "
        "COMPREHENSIVE than the draft, not a same-size reformat of it. Comprehensive means "
        "covering every real sub-part of the question and every relevant grounded fact — "
        "every deadline, every condition, every distinct case (e.g. initial filing window AND "
        "resubmission window AND participating-vs-non-participating, all three, not one). "
        "Pull in rag_chunks the draft didn't cite if they're relevant to the question — you "
        "have the same evidence react had, use more of it than the draft's first pass did. "
        "This applies to BOTH modes below — do not hold FACTUAL to a shorter bar just because "
        "the question sounds simple; if the corpus supports more, include more.\n\n"
        "The one hard limit: MAXIMIZE COVERAGE OF REAL, GROUNDED CONTENT — never maximize "
        "length by padding, restating, or inventing. Every added sentence must trace to a "
        "specific fact in reasoning_ledger, rag_chunks, or tool_outputs. If the grounding "
        "really is thin (rag_chunks/tool_outputs together don't cover much), say so honestly "
        "and stay short — comprehensiveness follows the evidence, it doesn't fabricate around "
        "gaps in it.\n\n"
        ...
        "Field rules:\n"
        "- direct_answer: length and content follow the mode-specific rules below. It should read as "
        "a fuller pass than react_draft, not a copy of it — expand and add coverage, don't just "
        "rephrase the same sentence.\n"
        ...
        "Add as many narrative sections as the grounded material actually supports — no fixed cap. "
        "For self-written sections: table=consistent columns; stats=2–5 KPIs; "
        ...
        "say so explicitly rather than inventing plausible-sounding specifics — comprehensiveness never overrides this.\n"
        "Mode-specific guidance (FACTUAL/BLENDED collapsed into one unified path — CANONICAL alone stays distinct; "
        "both modes get the same comprehensiveness bar, no section/bullet count cap on either):\n"
        "  FACTUAL: as many sections as the grounded material supports, 3 or more substantive bullets each where the "
        "source supports it. direct_answer covers every distinct sub-answer the corpus grounds — one sentence only if "
        "that is genuinely all the corpus supports, otherwise as many sentences as there are grounded sub-parts.\n"
        "  CANONICAL: as many sections as the grounded material supports, 3 or more substantive bullets each. "
        "direct_answer = full paragraphs covering every grounded sub-part, the more detailed of the two modes.\n"
    )
