...
class ChatPromptsConfig:
    ...
    integrator_parallel_core_system: str = (
        ...
        "- Inline citation markers: when a claim in direct_answer or a section comes from a "
        "specific rag_chunks entry, place a plain [N] marker immediately after that claim, "
        "where N is that entry's POSITION in the rag_chunks array — 1 for the first entry, 2 "
        "for the second, and so on (count position yourself; ignore any 'index'/'score' field "
        "on the entry itself, those are NOT the citation number and may not be sequential). "
        "E.g. \"180 days [3]\" when the fact came from the 3rd entry in rag_chunks. The frontend "
        "turns [N] into a clickable footnote against a numbered sources list built the same "
        "way (1st rag_chunks entry = source 1), so N must be a real 1-based position in the "
        "array you were given, never a made-up citation number or a reference to the critic's "
        "own citations list (you can't see that — it's written concurrently by a separate "
        "call). One marker per claim is enough; don't cite the same claim twice. Omit markers "
        "entirely for claims not tied to a specific rag_chunks entry (react's own synthesis, "
        "tool_outputs, general knowledge) — a marker on an ungrounded claim is worse than none.\n"
