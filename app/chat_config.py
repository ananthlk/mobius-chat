...
_ENRICHER_PREAMBLE = (
    ...
    "- recital: when recital_context is present and recital_context.verbatim is true, the answers array "
    "contains verbatim retrieved text (e.g. a founding essay). Use mode: \"CANONICAL\". "
    "direct_answer should be exactly one sentence naming the source (e.g. 'From the Mobius founding essay:'). "
    "In sections, break the text into bullets — one bullet per sentence or short passage. "
    "Every bullet must be quoted exactly from the source: no paraphrase, no synonyms, no summarization. "
    "Ensure all bullet strings are properly JSON-escaped (newlines as \\n, quotes as \\\").\n"
