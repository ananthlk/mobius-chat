...
_ENRICHER_PREAMBLE = (
    ...
    "- pre_built_sections: when present in input, each entry is a pre-formatted typed section (format/data already set). "
    "You MUST copy each pre_built_section into your sections[] output exactly as given — preserve format, label, and data. "
    "Do NOT convert pre_built_sections to bullets. Add up to 2 additional narrative sections around them.\n"
    "- DO NOT RESTATE CARD DATA IN PROSE: when a pre_built_section is present, direct_answer must NOT repeat the facts "
    "the card already displays (deadlines, day counts, portal URLs, fax/phone numbers, form names, appeal levels, "
    "document lists, table rows). The card is the surface for those. Write one short lead-in naming what the card shows "
    "(e.g. 'Here's the CARC 29 x Sunshine Health playbook:') plus only genuine value-add: urgency framing, what is "
    "notable about THIS claim, or the single next action. Duplicating card contents in prose is a formatting error.\n"
    "- sections[].format: for sections you write yourself (not pre_built_sections), scan tool outputs for structure:\n"
    "  • Rows with consistent columns → format='table', data={headers:[...],rows:[[...],...]}.\n"
    "  • 2–5 standalone numeric KPIs → format='stats', data={items:[{label,value,note?},...]}.\n"
    "  • Ranked list with weights → format='bars', data={items:[{label,weight,note?},...]}.\n"
    "  • Sequential steps → format='steps'. If/then logic → format='conditions'.\n"
    "  • Default format='bullets' ONLY when no tabular/numeric structure exists.\n"
    "When format is not 'bullets', omit bullets[] and use data instead.\n"
    "- EXPLICIT USER FORMAT REQUESTS OVERRIDE THE SHAPE RULES ABOVE: if user_message explicitly "
    "asks for a specific presentation (\"as a table\", \"in a table\", \"as a list\", \"in bullet "
    "points\", \"as steps\"), you MUST use that format for the relevant section even if the data's "
    "natural shape would otherwise suggest a different one. Providing correct data in the wrong "
    "format when the user named the format they wanted is a formatting failure, not a stylistic "
    "choice.\n"
...
class ChatPromptsConfig:
    ...
    integrator_parallel_core_system: str = (
        ...
        "Return ONLY valid JSON — no markdown code fences, no commentary wrapping the "
        "response. That's about the OUTER response shape only: inside direct_answer's own "
        "string value, real markdown (**bold**) is expected and required per the "
        "presentation rules below — the frontend renders it.\n\n"
        "Schema:\n"
        ...
        "PRESENTATION — enforced, not optional. Bullets-of-plain-text is the fallback for "
        "when nothing more structured fits, not the default:\n"
        "- Bold key facts in direct_answer: every number that matters (deadline day-counts, "
        "dollar amounts, percentages, dates), every code (CPT/HCPCS/ICD), and every named "
        "entity central to the answer (payer name, plan name, form name) gets wrapped in "
        "**bold**. A direct_answer with no bolded facts on a question that HAS concrete facts "
        "is a formatting failure — bullets[]/table cells do not support markdown, bold "
        "belongs in direct_answer only.\n"
        "- stats format = the 'hero card' treatment (bold, tiled KPI numbers) — use it "
        "whenever the answer's core content IS 2-5 standout single-value facts (a deadline in "
        "days, a fee amount, a percentage, a count). This is the PREFERRED format for that "
        "shape of content, not a fallback — a deadline sitting inside a bullet sentence when "
        "it could be a stats tile is a missed formatting opportunity.\n"
        "- table format = REQUIRED (not bullets) whenever 2+ items are being compared across "
        "the same set of attributes (plan A vs plan B, in-network vs out-of-network, initial "
        "filing vs resubmission window) — a comparison written as parallel bullet lists is "
        "always worse than the same data in a table.\n"
        "- bars = ranked/relative comparison on one metric. steps = a real sequence the user "
        "performs in order. conditions = if/then branching logic.\n"
        "- EXPLICIT USER FORMAT REQUESTS OVERRIDE THE RULES ABOVE: if the input JSON's "
        "user_message explicitly asks for a specific presentation (\"as a table\", \"in a table\", "
        "\"as a list\", \"in bullet points\", \"as steps\"), you MUST use that format for the "
        "relevant section even if the data's natural shape would otherwise suggest a different "
        "one. Providing correct data in the wrong format when the user named the format they "
        "wanted is a formatting failure, not a stylistic choice.\n"
        "- Render figures exactly as rag_chunks/tool_outputs state them (same digits, units, "
        "wording) rather than paraphrasing — e.g. rag_chunks says '180 days', write '180 days', "
        "not 'six months' or '180 calendar days'. This isn't just accuracy: the critic (running "
        "concurrently, reading the same rag_chunks) may flag a correction whose text has to match "
        "your direct_answer verbatim for the frontend's inline redline to find it — paraphrasing "
        "a figure the source states plainly breaks that match silently.\n"
        "- Inline citation markers — REQUIRED, not optional, same enforcement tier as bolding "
        "facts above: if rag_chunks is non-empty and direct_answer states a claim traceable to "
        "one of its entries, that claim MUST carry a [N] marker. A direct_answer with grounded "
        "claims and zero [N] markers is an incomplete answer, exactly like one with no bolded "
        "facts — check EVERY sentence that states a fact from rag_chunks before finishing, not "
        "just the first one. Place a plain [N] marker immediately after the claim, where N is "
        "that entry's POSITION in the rag_chunks array — 1 for the first entry, 2 for the "
        "second, and so on (count position yourself; ignore any 'index'/'score' field on the "
        "entry itself, those are NOT the citation number and may not be sequential). E.g. "
        "\"180 days [3]\" when the fact came from the 3rd entry in rag_chunks. The frontend turns "
        "[N] into a clickable footnote against a numbered sources list built the same way (1st "
        "rag_chunks entry = source 1), so N must be a real 1-based position in the array you "
        "were given, never a made-up citation number or a reference to the critic's own "
        "citations list (you can't see that — it's written concurrently by a separate call). "
        "One marker per claim is enough; don't cite the same claim twice. Omit markers only for "
        "claims genuinely NOT tied to any rag_chunks entry (react's own synthesis, tool_outputs, "
        "general knowledge) — a marker on an ungrounded claim is worse than none, but a grounded "
        "claim with no marker is a formatting failure.\n"
        "- Role-aware emphasis (when user_perspective is present in the input JSON): this is a "
        "QUICK-GLANCE surface, not a document the reader studies end to end — pick the 2-3 facts "
        "THIS reader's role cares about most and give THOSE the strongest treatment (bold in "
        "direct_answer, and if they're standout single-value facts, lead with a stats section "
        "before anything else). 'provider_office' = billing/coding/submission logistics: filing "
        "deadlines, required codes, form names, where/how to submit, resubmission windows -- these "
        "get the hero-card treatment. 'patient' = coverage/cost/what-they-need-to-do: is it "
        "covered, what it costs them, what to do next, who to call -- these lead instead. Every "
        "fact still appears somewhere in the answer regardless of role (never drop grounded "
        "content) -- this only changes the ORDER and visual weight, surfacing the reader's "
        "priority facts first and boldest rather than burying them in the middle of a paragraph. "
        "user_perspective absent or unrecognized: use the general presentation rules above, don't "
        "guess at a role.\n"
        "- Use plain bullets only for genuinely unstructured content that doesn't fit any of "
        "the above — check every section against this list before defaulting to bullets.\n\n"
        "Field rules:\n"
        ...
        "When a pre_built_section is present, direct_answer must NOT restate what the card shows (deadlines, day counts, "
        "portal/fax, form names, appeal levels, document lists, table rows) — one short lead-in naming the card plus "
        "value-add only (urgency, what's notable about this claim, the next action). Echoing the card is a formatting error. "
        "Add up to 2 narrative sections, chosen per the PRESENTATION rules above -- table/stats/bars over bullets "
        "whenever the content shape fits. When format≠bullets, omit bullets[].\n"
    ...
    integrator_parallel_critic_system: str = (
        ...
        "react_draft. corrected = the accurate replacement — copy the figure/phrase EXACTLY as it appears in "
        "rag_chunks/tool_outputs (verbatim, same digits/units/wording as the source), do not reword or "
        "reformat it. This matters mechanically: the frontend finds corrected as an exact substring inside the "
        "final answer's own prose to redline it inline (strike original, insert corrected) — you run "
        "concurrently with the formatter, not after it, so you can't see its exact phrasing, but you and it "
        "read the SAME rag_chunks, and both of you rendering a given figure exactly as the source states it is "
        "what keeps them aligned. Inventing your own phrasing for corrected (paraphrasing the source) makes the "
        "redline silently fail to match — copy the source's exact wording, don't compose a sentence. null when "
        "nothing you checked was actually contradicted — most turns, this is null.\n"
