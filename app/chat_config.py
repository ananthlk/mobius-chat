...
_ENRICHER_PREAMBLE = (
    ...
    "- sections[].format: BEFORE writing any section, scan the tool outputs and tool_section_hints for structure. "
    "These rules MUST be followed — they are not suggestions:\n"
    "  • If tool_section_hints contains an entry with section_format, honor it exactly for that section: "
    "use the declared format and populate data from table_headers/rows (for table) or items (for stats/bars).\n"
    "  • If content contains rows with consistent columns (comparisons, fee schedules, rate tables, peer benchmarks, MSA rates) "
    "→ format MUST be \"table\"; extract data.headers (string[]) and data.rows (string[][]) directly from the content.\n"
    "  • If content contains 2–5 standalone numeric KPIs with labels (denial rate, avg reimbursement, visit count) "
    "→ format MUST be \"stats\"; extract data.items [{label, value, note?}].\n"
    "  • If content contains a ranked list with relative frequency or weight "
    "→ format MUST be \"bars\"; extract data.items [{label, weight 0–1, note?}].\n"
    "  • \"steps\" — numbered sequential process; data.items (string[] — each step as a full sentence).\n"
    "  • \"conditions\" — if/then policy logic; data.items ([{condition, result}]).\n"
    "  • Default to \"bullets\" ONLY when no tabular or numeric structure is detectable. "
    "NEVER use \"bullets\" for data that has columns or consistent numeric rows.\n"
...
class ChatPromptsConfig:
    ...
    integrator_parallel_core_system: str = (
        ...
        "- sections[].format: MUST follow detection rules — scan tool outputs and tool_section_hints first. "
        "table = rows with consistent columns (rate data, comparisons, fee schedules, peer/MSA benchmarks); "
        "stats = 2–5 standalone numeric KPIs; bars = ranked list with weights; "
        "steps = sequential process; conditions = if/then logic. "
        "bullets ONLY when no tabular/numeric structure detected. "
        "Honor tool_section_hints[].section_format exactly when present. "
        "When format is not bullets, omit bullets[] and use data instead.\n"
