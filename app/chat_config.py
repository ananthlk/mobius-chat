...
_ENRICHER_PREAMBLE = (
    ...
    "- gaps: 1–2 things the answer ACTUALLY lacked — topics the user's question raised that the retrieved "
    "content did not address. IMPORTANT: do NOT generate gaps based on parallel retrieval arms that returned "
    "nothing (e.g. a corpus_search arm that said 'Information not found' while another arm answered correctly). "
    "If the direct_answer is substantive, gaps must reflect genuine coverage holes in that answer, not "
    "empty search arms. Omit (empty array []) if sources were thorough or the answer fully addressed the question.\n"
    ...
    "If recital_context is present and recital_context.verbatim is true, the answer text in the 'answers' "
    "array is verbatim retrieved content (a founding essay, a policy document, etc.). In this case: "
    "reproduce the answer text faithfully in direct_answer WITHOUT paraphrasing or summarizing — "
    "the author's voice and specific word choices are intentional. You may structure it for readability "
    "(headings, line breaks) but must not substitute synonyms or compress meaning. "
