...
_ENRICHER_PREAMBLE = (
    ...
    "- sections[].format: BEFORE writing any section, scan tool_section_hints and tool outputs for structure. "
    "These rules MUST be followed — they are not suggestions:\n"
    "  • If tool_section_hints contains an entry with section_format='table', output that section as:\n"
    "    {\"intent\":\"...\",\"label\":\"...\",\"format\":\"table\",\"data\":{\"headers\":[...],\"rows\":[[...],...]}}\n"
    "    Use table_headers as data.headers and rows as data.rows. Do NOT use ui_blocks. Do NOT use bullets.\n"
    "  • If tool_section_hints contains an entry with section_format='stats', output:\n"
    "    {\"intent\":\"...\",\"label\":\"...\",\"format\":\"stats\",\"data\":{\"items\":[{\"label\":\"...\",\"value\":\"...\"},...]}}\n"
    "  • If content has rows with consistent columns (rate tables, comparisons, benchmarks) → format='table'.\n"
    "  • If content has 2–5 standalone numeric KPIs → format='stats'; data.items [{label, value, note?}].\n"
    "  • If content has a ranked list with frequency/weight → format='bars'; data.items [{label, weight, note?}].\n"
    "  • 'steps' — sequential process; data.items (string[]). 'conditions' — if/then; data.items [{condition,result}].\n"
    "  • Default format='bullets' ONLY when no tabular/numeric structure exists. NEVER bullets for columnar data.\n"
    "When format is not 'bullets', omit bullets[] entirely and use data instead.\n"
...
class ChatPromptsConfig:
    ...
    integrator_parallel_core_system: str = (
        ...
        "- sections[].format + data: when tool_section_hints has section_format='table', the section MUST be "
        "{\"format\":\"table\",\"data\":{\"headers\":[...],\"rows\":[[...],...]}} — no ui_blocks, no bullets. "
        "table=consistent columns; stats=2–5 KPIs (data.items); bars=ranked+weights; steps=sequence; conditions=if/then. "
        "bullets only when no columnar/numeric structure exists. When format≠bullets, omit bullets[].\n"
