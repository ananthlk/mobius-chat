...
1b. **Retry protocol — relax, then reframe. Never blind-retry.** rag already runs its own internal strategy escalation (BM25/vector/web/fact-store) INSIDE one call — calling it again with the same or cosmetically-reworded query changes nothing server-side and gains zero new information. Every rag tool result carries a "RAG signal:" line (status, citable_required, chunks), and the [Evidence Ledger] block above shows gap_status for this exact pattern — read both before deciding your next move:
    ...
    (commit 4268736, 2026-08-06: this protocol replaced a dead 5-arm-bandit/mode-cascade retry scheme after the H0036 10-round regression — the prior dispatch ignored every mode/k parameter the retry logic varied, so every round sent a functionally identical request and the model's only real lever was cosmetic query rewording. Inferred link between this commit and this block version from timing + subject matter, not a stated cross-reference — same day, ~1h45m apart, same author cluster; check the commit if the chain matters to you.)
...
def _call_llm_json(
    ...
    *,
    ...
    if (stage or "").startswith("react_"):
        ...
        #
        # 🔴 THIS FILE IS V1'S AND MUST STAY PURE. Ananth, 2026-09-12: "make
        # sure v2 does not contaminate v1 in anyways.. v1 should stay pure for
        # a/b compares.. including prompts and everything", then "create a
        # prompts v2 so that it is clean too".
        #
        # I raised this floor to 8000 for v2's benefit and shipped it UNGATED:
        # _call_llm_json serves both arms, so v1's truncation behaviour moved
        # underneath the comparison and every A/B number after it would have
        # been measuring two changes at once. Reverted to the value v1 was
        # measured under.
        #
        # v2 does NOT need this to change: it passes its own max_tokens (see
        # app/pipeline/v2/prompts.py), and a floor never lowers a caller's
        # explicit ask. Nothing about v2 belongs in this file.
