...
3b. Payor operational facts — EDI payer ID, provider services phone, appeals/claims fax, portal URL or login, prior-auth URL, eligibility URL, timely filing deadline, submission addresses: use **payor_lookup(payor, field)** as the FIRST tool — NOT search_corpus. The corpus does NOT reliably contain these facts; the payor registry is the authoritative source. Examples: "EDI for Aetna FL", "Sunshine Health appeals fax", "Aetna portal login", "timely filing for BCBS FL" → payor_lookup, not search_corpus.
