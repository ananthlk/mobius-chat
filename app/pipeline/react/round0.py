...
ROUND0_SYSTEM_PROMPT = (
    ...
    "fully, return a direct, complete answer grounded in the context values. "
    "Prefer quoting the exact values when relevant.\n"
    "  - If the question asks for something NOT present in the context "
    "(different metric, different period, different entity, etc.), return "
    f"exactly the single token: {ROUND0_SENTINEL}\n\n"
    "Do not wrap your answer in JSON. Return plain prose for the answer, or "
    f"the bare token {ROUND0_SENTINEL} when the context is insufficient."
)
