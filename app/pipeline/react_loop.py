...
from app.pipeline.react import appeals_dispatch as _appeals_dispatch
...
def _execute_tool(
    ...
    # Extracted to react/appeals_dispatch.py (Phase 1i pass, 2026-09-16) —
    # 590 lines, lifted whole. The membership test lives with the
    # implementation so there is one tool list, not two.
    if tool in _appeals_dispatch.APPEALS_TOOLS:
        _ap = _appeals_dispatch.dispatch(tool, inputs, ctx, emit)
        if _ap is not None:
            return _ap
                    ...
                    try:
                        _pp_critic_raw = _call_llm_json(
                            ...
                            _pp_build_critic_msg(
                                ...
                                last_turns=getattr(ctx, "last_turns", None),
                                previous_thread_summary=getattr(ctx, "previous_thread_summary", None),
                                last_turn_sources=getattr(ctx, "last_turn_sources", None),
                            ),
