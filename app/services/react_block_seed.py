...
def _react_block_specs() -> list[BlockSpec]:
    ...
    return [
        ...
        # v8 (2026-09-10, Governor seat thread): adds an inline citation to
        # rule 1b's retry protocol, closing the one entry in this block that
        # had no traceable why. Content-only change (a citation, not a new
        # rule) -- no directive/behavior change. See commit 4268736 for the
        # H0036 incident this protocol replaced. (v7 exists in the DB as a
        # byte-off duplicate of this content -- an extra trailing newline
        # from a manual publish before this seed file was updated to match
