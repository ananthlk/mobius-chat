...
class TestNoChatEndpointsInlineInMain:
    ...
    def test_previously_extracted_paths_are_not_back(self):
        """Concrete regression: the 51 paths already moved (1a-1d) must not
        reappear inline in main.py. This is the strict lower bound — even if
        the allowed-prefixes list gets too permissive, these specific URLs
        can never come back."""
        already_moved_prefixes = (
            "/chat/history/",
            "/chat/feedback/",
        ...
        offenders = [
            ...
            if any(path.startswith(p) for p in already_moved_prefixes)
        ]
        assert not offenders, (
            "Paths already extracted in Phase 1a-1d have reappeared in main.py:\n"
            + "\n".join(f"  main.py:{ln}  {m}  {p}" for ln, m, p in offenders)
        )
...
class TestSharedHelpersConsolidated:
    ...
    def test_no_inline_task_manager_base_in_routers(self):
        """Routers must import from _common, not redefine the helper locally."""
        offenders = []
        for router_name in ("credentialing.py", "roster.py"):
            p = CHAT_REPO_ROOT / "app" / "api" / router_name
            text = p.read_text()
            for ln_no, line in enumerate(text.splitlines(), start=1):
                stripped = line.lstrip()
                if stripped.startswith("def _task_manager_base"):
                    offenders.append(f"{router_name}:{ln_no}")
