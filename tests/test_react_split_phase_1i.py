...
class TestReactLoopRatchet:
    ...
    MAX_REACT_LOOP_LOC = 2_560  # 2026-07-09: +47 LOC for task-context threading through integrate stage (task_aware_integrator) and @-mention plumbing in react_loop.  # 2026-05-29: +20 LOC parse-failure prose fallback broadened (any round>=3, not guidance-only); llm_health NameError fix; prompts JSON footer.  # 2026-05-11: +80 LOC for three task-mode bug fixes — (1) guidance-round prose parse fallback (~20 LOC): when LLM emits prose instead of JSON in guidance mode, use it directly as the answer; (2) task-mode empty-answer fallthrough guard (~18 LOC): prevent tool=null/answer="" from defaulting to search_corpus via `tool or "search_corpus"` at dispatch; (3) post-loop task-mode escalation guard (~16 LOC): intercept "verified answer/searching" escalation for task mode.  # 2026-05-04: +33 LOC for 5-arm bandit (precision→recall→hybrid→google→llm_direct) with arm elimination on failure. google_search handler marks "google" arm tried. Previously +27 LOC for corpus exhaustion sentinel.  # 2026-04-24: +130 LOC for retrieval taxonomy (Sprint 2 #0.2): _TOOL_ALIASES (~30 LOC), _normalize_tool_name (~10 LOC), precision_search dispatch (~80 LOC), comment header (~10 LOC). R2 refactor still tracked.   # 2026-04-23: +200 LOC absorbing three independent features landed this sprint — system_context Round 0 integration (~30 LOC), cache-assist seed_tool_results consumption (~15 LOC), critic-skip on pre-audited cache hits including the _cache_preaudited_critic_skip gate (~60 LOC + docs). R2 refactor (extract _execute_tool to react/dispatcher.py) is tracked on the production-readiness plan and will claw this back to ~1_500 when it lands.
