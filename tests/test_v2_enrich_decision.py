...
from app.pipeline.v2.enrich import decide, should_reopen
...
def test_always_on_still_records_the_criteria():
    ...
    d = decide(answer="an answer", facts=[_F(True), _F(False)],
...
def test_no_answer_is_a_prerequisite_not_a_policy():
    ...
    prose about nothing."""
    d = decide(answer="   ", facts=(), open_gaps=())
...
def test_the_gating_shape_exists_and_is_reachable(monkeypatch):
    ...
    grounded_complete = decide(answer="a", facts=[_F(True)], open_gaps=(),
                               is_complete=True)
    assert not grounded_complete.run_critic
    unsupported = decide(answer="a", facts=[_F(False)], open_gaps=())
...
def test_reopen_affordability_is_computed_from_budget_not_asked():
    d = decide(answer="a", elapsed_s=20.0, promise_s=31.0, rounds_left=2,
               round_cost_s=5.0)
    assert d.reopen_affordable
    broke = decide(answer="a", elapsed_s=29.0, promise_s=31.0, rounds_left=2,
                   round_cost_s=5.0)
    assert not broke.reopen_affordable


def test_no_rounds_left_is_not_affordable_however_fast_the_turn_was():
    assert not decide(answer="a", elapsed_s=1.0, promise_s=31.0,
...
def test_unknown_budget_is_not_treated_as_affordable():
    ...
    assert not decide(answer="a", rounds_left=2).reopen_affordable


# ── reopening stays strict ──────────────────────────────────────────────────

def test_reopen_only_on_objectively_unfinished_work():
    d = decide(answer="a", elapsed_s=1.0, promise_s=31.0, rounds_left=2,
               round_cost_s=1.0)
...
def test_unaffordable_reopen_hands_the_choice_to_the_user():
    ...
    d = decide(answer="a", elapsed_s=30.0, promise_s=31.0, rounds_left=1,
               round_cost_s=9.0)
    ok, why = should_reopen({"parts": [{"part": "UHC", "status": "not_attempted"}]}, d)
